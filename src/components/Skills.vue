<template>
    <div>
        <h1 class="h2 text-center">Mes compétences</h1>
		<hr>
		<ul class="list-group list-group-flush">
			<li class="list-group-item">Programmer une application web en <strong>PHP orienté objet</strong></li>
			<li class="list-group-item">Construire une application Web avec <strong>Symfony</strong></li>
			<li class="list-group-item">Créer et gérer des requêtes en base de données avec <strong>MySQL</strong></li>
			<li class="list-group-item">Développer une application en <strong>JavaScript</strong> (JQuery, Node.js et Vue.js)</li>
			<li class="list-group-item">Mettre en place et se connecter à une <strong>API</strong>, requêtes <strong>AJAX</strong></li>
			<li class="list-group-item">Déployer un site avec <strong>WordPress</strong></li>
			<li class="list-group-item">Construire un site web <strong>Responsive</strong></li>
			<li class="list-group-item">Utiliser l’outil de gestion de versions <strong>Git</strong></li>
			<li class="list-group-item">Référencement <strong>SEO</strong></li>
		</ul>
		
    </div>
</template>

<script>
export default {

}
</script>

<style>

</style>